...
namespace Telegram.Bot
{
	internal partial class Database : IDisposable
	{
		...
		private readonly TelegramBotClient.State _state;

		public Database(DbConnection connection, string[] sqlCommands, TelegramBotClient.State state)
		{
		...
		private Chat? DoLoadChat(long id)
		{
			...
			if (!reader.Read()) return null;
			var flags = reader.GetInt32(1);
			var type = (Types.Enums.ChatType)reader.GetInt32(5);
			...
			return new Chat
			{
				...
				AccessHash = reader.GetInt64(0),
				Type = type,
				Title = type == Types.Enums.ChatType.Private ? null : firstName,
				FirstName = type == Types.Enums.ChatType.Private ? firstName : null,
				LastName = type == Types.Enums.ChatType.Private ? reader.GetString(3).NullIfEmpty() : null,
				Username = reader.GetString(4).NullIfEmpty(),
				IsForum = (flags & 1) != 0,
			};
		}
		...
		private void DoSaveChat(Chat chat)
		{
			...
			param[3].Value = (chat.Type == Types.Enums.ChatType.Private ? chat.FirstName : chat.Title) ?? "";
